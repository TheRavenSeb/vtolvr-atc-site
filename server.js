...
app.post('/api/chart-maker/generate', authHandler.AdminOnly("mod"), async (req, res) => {
  try {
    const {
      template = 'large.png',
      airportName = 'SHORK AIRPORT',
      airportAbbr = 'ABRV',
      cityState = 'CITY, STATE',
      airportCode = 'AL-289 (FAA)',
      runways = [],
      callsign = 'Shork Airbase:241.5',
      generalFrequency = 'General: 254.0'
    } = req.body || {};

    const safeTemplate = String(template);
    if (!['large.png', 'medium.png'].includes(safeTemplate)) {
      return res.status(400).json({ error: 'Invalid template selected.' });
    }

    const templatePath = path.join(__dirname, 'assets', 'diagrams', safeTemplate);
    if (!fs.existsSync(templatePath)) {
      return res.status(404).json({ error: 'Template image not found.' });
    }

    if (!Array.isArray(runways) || runways.length === 0) {
      return res.status(400).json({ error: 'At least one runway is required.' });
    }
    ...
    // Font sizes scaled to template
    const headerSize = Math.max(24, Math.round(w * 0.04));
    const titleSize = Math.max(28, Math.round(w * 0.045));
    const codeSize = Math.max(16, Math.round(w * 0.022));
    const runwaySize = Math.max(48, Math.round(w * 0.075));
    const freqSize = Math.max(20, Math.round(w * 0.028));

    // TOP LEFT: Airport Diagram title
    ctx.font = `bold ${titleSize}px Arial`;
    ctx.textAlign = 'left';
    ctx.fillText('AIRPORT DIAGRAM', pad, pad + 12);

    // TOP CENTER: Airport Code
    ctx.font = `500 ${codeSize}px Arial`;
    ctx.textAlign = 'center';
    ctx.fillText(String(airportCode).toUpperCase(), w / 2, pad + 12);

    // TOP RIGHT: Airport Name and City/State
    ctx.font = `bold ${headerSize}px Arial`;
    ctx.textAlign = 'right';
    const airportName_upper = String(airportName).toUpperCase();
    const airportAbbr_upper = String(airportAbbr).toUpperCase();
    ctx.fillText(`${airportName_upper}(${airportAbbr_upper})`, w - pad, pad + 8);

    ctx.font = `500 ${codeSize}px Arial`;
    ctx.fillText(String(cityState).toUpperCase(), w - pad, pad + 24);

    // RUNWAY LABELS (positioned on sides, distributed evenly)
    ctx.font = `bold ${runwaySize}px Arial`;
    ctx.textAlign = 'center';
    ctx.fillStyle = '#000000';
    ...
    const verticalSpacing = availableHeight / (runwayCount + 1);

    // Left side runways
    runways.forEach((runway, index) => {
      const yPos = topMargin + (index + 1) * verticalSpacing;
      ctx.fillText(String(runway).toUpperCase(), w * 0.08, yPos);
    });

    // RIGHT SIDE: Callsign and Frequency
    ctx.font = `bold ${freqSize}px Arial`;
    ctx.textAlign = 'right';
    const freqX = w - pad - 20;
    const freqTopMargin = pad + 40;
    ctx.fillText(String(callsign), freqX, freqTopMargin);
    ctx.fillText(String(generalFrequency), freqX, freqTopMargin + freqSize + 8);

    // BOTTOM: Mirror of top
    ctx.font = `bold ${titleSize}px Arial`;
    ctx.textAlign = 'left';
    ctx.fillText('AIRPORT DIAGRAM', pad, h - pad - 8);

    ctx.font = `500 ${codeSize}px Arial`;
    ctx.textAlign = 'center';
    ctx.fillText(String(cityState).toUpperCase(), w / 2, h - pad - 8);

    ctx.font = `bold ${headerSize}px Arial`;
    ctx.textAlign = 'right';
    ctx.fillText(`${airportName_upper}(${airportAbbr_upper})`, w - pad, h - pad - 8);
    ...
    return res.json({
      ...
      filename: `${String(airportAbbr || 'chart').toLowerCase()}-airport-diagram.png`,
      imageData: `data:image/png;base64,${base64Png}`
    });
  } catch (error) {
    console.error('Error generating chart image:', error);
    return res.status(500).json({ error: 'Failed to generate chart image.' });
  }
});
